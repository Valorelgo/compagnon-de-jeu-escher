...
function quickEquipStashItem(itemName, fighterId) {
    ...
    if (isWeapon) {
        ...
        if (foundDbW && foundDbW.specific_to) {
            let matchChar = m.charId === foundDbW.specific_to;
            let matchType = (m.type || []).some(t => String(t).toLowerCase() === String(foundDbW.specific_to).toLowerCase());
            if (!matchChar && !matchType) return showToast(`"${itemName}" est une arme associée réservée : ${m.customName} ne remplit pas le critère de réservation.`, "error");
        }
