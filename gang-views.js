...
function renderMenu(container) {
    ...
    container.innerHTML = `
        <div class="card home-hero">
            <div class="home-hero-emblem">☠️</div>
            <div class="home-hero-title">Sous-Monde</div>
            <div class="home-hero-subtitle">Compagnon de jeu — Gangs Escher</div>
            <p class="home-hero-flavor">
                Dans les profondeurs toxiques du Sous-Monde, les gangs Escher font de la vitesse, du poison
                et de la chimie leurs armes de prédilection. Gérez votre bande, son évolution de campagne
                en campagne, et menez vos guerrières au combat.
            </p>

            <div class="home-actions-grid">
                <div class="home-action-card" onclick="navigate('gang-create')">
                    <div class="home-action-icon">⚔️</div>
                    <div class="home-action-title">Nouveau Gang</div>
                    <div class="home-action-desc">Fondez un gang Escher et recrutez vos premières guerrières.</div>
                </div>
                <div class="home-action-card" onclick="appState.mode='campaign'; navigate('gang-select')">
                    <div class="home-action-icon">📜</div>
                    <div class="home-action-title">Suivi de Campagne</div>
                    <div class="home-action-desc">Reprenez un gang existant : roster, territoires, post-cycle et parties.</div>
                </div>
            </div>

            <div class="home-import-row">
                <button onclick="importGang()">⇪ Importer un gang Escher (.json)</button>
            </div>
        </div>
    `;
}
...
function renderGangSelect(container) {
    ...
    let escherGangNames = Object.keys(savedGangs).filter(name => {
        const g = savedGangs[name];
        return !g.faction || g.faction === APP_GANG_FACTION;
    });
    if(escherGangNames.length === 0) {
        html += `<p>Aucun gang sauvegardé.</p>`;
    } else {
        for(let name of escherGangNames) {
...
function loadGang(name) {
    ...
    if (g.faction && g.faction !== APP_GANG_FACTION) {
        return showToast(`"${name}" est une liste ${g.faction}, pas Escher. Ce programme ne peut pas l'ouvrir.`, "error");
    }
    currentGang = g;
