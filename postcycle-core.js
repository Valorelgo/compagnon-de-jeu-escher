...
function actionTacticalDevelopment() {
    ...
    if (availablePool.length === 0) {
        return showToast(`Toutes les cartes tactiques (${allPool.length}/${allPool.length}, générique + Escher) ont déjà été découvertes par le gang !`, "error");
    }

    let genCount = getTacticsPoolByCategory('generique').filter(t => !ownedIds.includes(t.id)).length;
    let escherCount = getTacticsPoolByCategory('escher').filter(t => !ownedIds.includes(t.id)).length;
    ...
    let html = `
        ...
        <p><small>Pour chaque guerrier choisi, <strong>1 nouvelle carte tactique</strong> est tirée aléatoirement dans la catégorie choisie, parmi les cartes que le gang ne possède pas encore (aucun doublon).</small></p>
        <div style="background:#11131c; border:1px solid #282b42; border-radius:6px; padding:8px 12px; margin-bottom:12px; font-size:13px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
            ...
            <span>Restantes : <strong style="color:var(--accent-cyan);">Générique ${genCount}</strong> / <strong style="color:var(--accent-cyan);">Escher ${escherCount}</strong></span>
        </div>
        <p><small style="color:#e74c3c;">⚠️ Règle : 1 seule action post-cycle par guerrier.</small></p>
        <hr style="margin:10px 0; border-color:#333;">
        <div style="max-height:50vh; overflow-y:auto;">
    ...
    eligible.forEach(m => {
        ...
        html += `
            <div style="background:#111; border:1px solid #333; padding:8px 12px; border-radius:5px; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; ${(isBusyOther || isAlreadyDone) ? 'opacity:0.5;' : ''}">
                <div>
                    <strong style="color:var(--accent-cyan);">${m.customName || m.charName}</strong> 
                    <small style="color:#aaa;">(${(m.type || []).join(', ')})</small>
                ...
                <div style="display:flex; align-items:center; gap:8px;">
                    ${!isAlreadyDone && !isBusyOther ? `<select id="tactics-cat-fighter-${m.id}" style="margin:0; font-size:12px;">
                        <option value="generique">Générique (${genCount})</option>
                        <option value="escher" ${escherCount === 0 ? 'disabled' : ''}>Escher (${escherCount})</option>
                    </select>` : ''}
                    ${isAlreadyDone 
...
function confirmTacticalDevelopment() {
    ...
    checkedBoxes.forEach(cb => {
        ...
        let catSel = document.getElementById(`tactics-cat-fighter-${fId}`);
        let category = (catSel && catSel.value === 'escher') ? 'escher' : 'generique';

        let categoryPool = getTacticsPoolByCategory(category).filter(t => !ownedIds.includes(t.id));
        if (categoryPool.length === 0) {
            // Catégorie choisie épuisée pour ce guerrier : on retombe sur l'autre
            // catégorie si elle a encore des cartes disponibles, plutôt que de
            // ne rien tirer du tout.
            let fallbackCategory = category === 'escher' ? 'generique' : 'escher';
            categoryPool = getTacticsPoolByCategory(fallbackCategory).filter(t => !ownedIds.includes(t.id));
            category = fallbackCategory;
            if (categoryPool.length === 0) return;
        }

        let randIdx = Math.floor(Math.random() * categoryPool.length);
        let pickedCard = JSON.parse(JSON.stringify(categoryPool[randIdx]));
        pickedCard.category = category;

        currentGang.tactics.push(pickedCard);
        ownedIds.push(pickedCard.id);
        ...
        resHtml += `
            <div style="border:1px solid #444; background:#111; border-radius:6px; padding:10px 12px; margin-bottom:10px; border-left:4px solid var(--accent-purple);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                    ...
                    <small style="color:#aaa; background:#1e2130; padding:2px 8px; border-radius:4px;">${item.card.category === 'escher' ? 'Escher' : 'Générique'} — Générée par : <strong>${item.fighterName}</strong></small>
                </div>
