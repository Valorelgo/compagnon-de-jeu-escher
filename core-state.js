...
function migrateSavedGangsIfNeeded() {
    const storedVersion = parseInt(localStorage.getItem('escherGangsSchemaVersion'), 10) || 0;
    ...
    localStorage.setItem('escherGangsSchemaVersion', String(GANGS_SCHEMA_VERSION));
    saveGangs();
}
...
function quarantineForeignFactionGangs() {
    ...
    let quarantine = {};
    try {
        quarantine = JSON.parse(localStorage.getItem('escherGangs_otherFactions')) || {};
    } catch (e) {
        quarantine = {};
    }
    ...
    localStorage.setItem('escherGangs_otherFactions', JSON.stringify(quarantine));
...
function checkMountedWeaponRestriction(fighter, weapon) {
    ...
    // Exception Escher cutter : le cutter heavy stubbers reste autorisé malgré
    // son trait Lourd, contrairement aux autres armes lourdes.
    if (weapon.id === 'wpn_cutter_heavy_stubbers' && (fighter.equipment || []).some(e => e && e.id === 'eq_escher_cutter')) {
        return null;
    }
...
// ==========================================
// CARTES TACTIQUES : POOLS PAR CATÉGORIE (GÉNÉRIQUE / ESCHER)
// ==========================================
// Les Escher ont leur propre pool de cartes tactiques (db.tactics_escher),
// distinct du pool générique (db.tactics). Toute génération de carte (recrutement
// d'un leader/champion, développement tactique post-cycle) doit donc demander
// dans quelle catégorie piocher. Ces utilitaires sont partagés entre
// fighter-skills-tactics.js et postcycle-core.js.
function getTacticsPoolByCategory(category) {
    if (category === 'escher') return (typeof db !== 'undefined' && db.tactics_escher) ? db.tactics_escher : [];
    return (typeof db !== 'undefined' && db.tactics) ? db.tactics : [];
}

// Pool combiné (générique + Escher), utilisé pour résoudre une carte déjà
// possédée par son id (affichage, comptage du deck total...).
function getAllTacticsPool() {
    let generique = (typeof db !== 'undefined' && db.tactics) ? db.tactics : [];
    let escher = (typeof db !== 'undefined' && db.tactics_escher) ? db.tactics_escher : [];
    return generique.concat(escher);
}

// Pioche 1 carte par entrée de `categories` (ex: ['generique','escher']),
// dans le pool correspondant, sans jamais tirer de doublon (ni avec les cartes
// déjà possédées par le gang, ni entre les cartes tirées dans le même lot).
// Retourne le tableau des cartes effectivement tirées.
function drawTacticsForGang(gang, categories) {
    if (!gang || !categories || categories.length === 0) return [];
    if (!gang.tactics) gang.tactics = [];
    if (typeof ensureNoDuplicateTactics === 'function') ensureNoDuplicateTactics(gang);

    let ownedIds = gang.tactics.map(t => (typeof t === 'string' ? t : t.id));
    let drawn = [];
    categories.forEach(cat => {
        let pool = getTacticsPoolByCategory(cat).filter(t => !ownedIds.includes(t.id));
        if (pool.length === 0) return;
        let randIdx = Math.floor(Math.random() * pool.length);
        let cardCopy = JSON.parse(JSON.stringify(pool[randIdx]));
        cardCopy.category = cat;
        gang.tactics.push(cardCopy);
        drawn.push(cardCopy);
        ownedIds.push(cardCopy.id);
    });
    return drawn;
}

// Fenêtre générique demandant, pour chacune des `count` cartes à générer, dans
// quelle catégorie piocher (Générique ou Escher). Appelle `confirmFnName`
// (nom de fonction globale, sans argument) au clic sur "Piocher" : cette
// fonction doit relire les sélections via readTacticsCategorySelections().
function openTacticsCategoryChoiceModal(count, confirmFnName) {
    let genPool = getTacticsPoolByCategory('generique');
    let escherPool = getTacticsPoolByCategory('escher');
    let rows = '';
    for (let i = 0; i < count; i++) {
        rows += `
            <div style="background:#111; border:1px solid #333; border-radius:6px; padding:10px 12px; margin-bottom:8px;">
                <div style="font-size:13px; color:#ccc; margin-bottom:6px;">Carte ${i + 1} / ${count} — catégorie à piocher :</div>
                <select id="tactics-cat-${i}" style="margin:0;">
                    <option value="generique">Générique (${genPool.length} cartes)</option>
                    <option value="escher" ${escherPool.length === 0 ? 'disabled' : ''}>Escher (${escherPool.length} cartes)</option>
                </select>
            </div>
        `;
    }
    let html = `
        <p style="font-size:13px; color:#ccc; margin-bottom:10px;">
            Pour chaque carte à générer, choisissez la catégorie dans laquelle piocher : les cartes tactiques
            <strong>génériques</strong>, ou les cartes <strong>Escher</strong> propres à la maison.
        </p>
        ${rows}
        <button class="btn btn-cyan" style="width:100%; padding:10px; font-weight:bold; margin-top:6px;" onclick="${confirmFnName}()">🎴 Piocher</button>
    `;
    if (typeof openModal === 'function') openModal("🎴 Choix de la catégorie de tirage", html);
}

function readTacticsCategorySelections(count) {
    let categories = [];
    for (let i = 0; i < count; i++) {
        let sel = document.getElementById(`tactics-cat-${i}`);
        categories.push(sel && sel.value === 'escher' ? 'escher' : 'generique');
    }
    return categories;
}
