...
function openWeaponSelectModal() {
    ...
    if (isCampaign && !isHiveScum && !isMercOrBeast) {
        ...
        let availableWeapons = currentGang.stash.filter(item => {
            ...
            // Armes associées (specific_to) : une arme réservée reprise depuis le
            // stash ne peut être proposée qu'aux guerriers répondant au critère
            // de réservation (même personnage ou même type que celui d'origine).
            let itemName = (typeof item === 'object') ? item.name : item;
            let dbW = db.weapons.find(w => w.name === itemName);
            if (dbW && dbW.specific_to) {
                let matchChar = tempFighter.charId === dbW.specific_to;
                let matchType = (tempFighter.type || []).some(t => String(t).toLowerCase() === String(dbW.specific_to).toLowerCase());
                if (!matchChar && !matchType) return false;
            }
            return true;
        });
