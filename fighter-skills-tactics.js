    ...
    input.onchange = e => {
        ...
        reader.onload = (event) => {
            try {
                ...
                // Ce programme ne gère que des listes Escher : on rejette tout fichier
                ...
                // protection) est accepté et tagué Escher, par compatibilité.
                if (gang.faction && gang.faction !== APP_GANG_FACTION) {
                    showToast(`Ce fichier est une liste "${gang.faction}", pas une liste Escher. Ce programme n'importe que des listes Escher.`, "error");
                    return;
                }
                ...
                saveGangs();
                showToast("Gang Escher importé avec succès !", "success");
...
// État temporaire portant les infos du combattant en attente de choix de
// catégorie(s) pour son tirage de cartes tactiques (recrutement leader/champion).
let pendingTacticsRecruitDraw = null;
...
function handleRecruitTactics(fighter, isPostCycle) {
    ...
    if (availablePool.length === 0) {
        let modalTitle = "🎴 Deck Tactique Déjà Complet";
        let html = `
            <div style="padding:4px;">
                ...
                <div style="background:#111; border:1px solid #444; border-radius:6px; padding:12px; margin-bottom:14px; color:#ccc; font-size:13px;">
                    ...
                    Toutes les cartes tactiques du jeu (${allPool.length} / ${allPool.length}, générique + Escher) font déjà partie du deck de votre gang. Aucune nouvelle carte supplémentaire ne peut être tirée.
                </div>
        ...
        `;
        if (typeof openModal === 'function') openModal(modalTitle, html);
        return;
    }

    pendingTacticsRecruitDraw = { fighterId: fighter.id, count, isLeader, isChampion, fName, fRole };
    openTacticsCategoryChoiceModal(count, 'confirmRecruitTacticsDraw');
}

function confirmRecruitTacticsDraw() {
    if (!pendingTacticsRecruitDraw || !currentGang) return;
    let { count, isLeader, isChampion, fName, fRole } = pendingTacticsRecruitDraw;
    let categories = readTacticsCategorySelections(count);
    let drawnCards = drawTacticsForGang(currentGang, categories);
    pendingTacticsRecruitDraw = null;
    if (drawnCards.length > 0) saveGangs();
    showTacticsDrawResultModal(fName, fRole, isLeader, isChampion, drawnCards);
}
window.confirmRecruitTacticsDraw = confirmRecruitTacticsDraw;

// Affiche le résultat d'un tirage de cartes tactiques (recrutement leader/champion).
function showTacticsDrawResultModal(fName, fRole, isLeader, isChampion, drawnCards) {
    if (!currentGang) return;
    let allPool = getAllTacticsPool();
    let modalTitle = "🎴 Nouvelles Cartes Tactiques Débloquées";
    let html = `
        <div style="padding:4px;">
            <div style="background:#11131c; border:1px solid #282b42; border-radius:6px; padding:12px; margin-bottom:14px;">
                <div style="font-size:14px; color:#fff; margin-bottom:4px;">
                ...
                ${drawnCards.map((card, idx) => `
                    <div style="background:#111; border:1px solid #444; border-left:4px solid var(--accent-cyan); border-radius:6px; padding:10px 12px; margin-bottom:10px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                            ...
                            <span style="font-size:11px; color:#aaa; background:#1e1e2f; padding:2px 8px; border-radius:4px;">${card.category === 'escher' ? 'Escher' : 'Générique'} — Carte ${idx + 1}/${drawnCards.length}</span>
                        </div>
...
function initApp() {
    ...
    if (typeof _quarantinedGangsCount !== 'undefined' && _quarantinedGangsCount > 0) {
        ...
        showToast(`${n} liste${n > 1 ? 's' : ''} d'une autre faction que Escher détectée${n > 1 ? 's' : ''} et retirée${n > 1 ? 's' : ''} de cette appli (conservée${n > 1 ? 's' : ''}, non affichée${n > 1 ? 's' : ''} ici).`, "info");
...
window.drawTacticsForGang = drawTacticsForGang;
window.getAllTacticsPool = getAllTacticsPool;
